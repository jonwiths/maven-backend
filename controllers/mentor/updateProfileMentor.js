...
const updateProfileMentorAbout = (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, userInfo) => {
    if (err) return res.status(500).json(err);
      ...
      const q =
        'UPDATE `heroku_713c4886f766b8c`.mentor_about SET `bio` = ?, `sex` = ?, `age` = ?, `phone_number` = ?, `email` = ?, address = ?,`yrs_exp` = ?,`current_job` = ? WHERE `mentor_id` = ?;';
...
const updateProfileMentorExperience = (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, userInfo) => {
    if (err) return res.status(500).json(err);
      ...
      // console.log(userInfo.id);
      const q =
        'UPDATE  `heroku_713c4886f766b8c`.mentor_education SET current_job = ? , current_job_started = ?, company = ? WHERE mentor_id = ?';
...
const updateProfileMentorEducation = (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, userInfo) => {
    if (err) return res.status(500).json(err);
      ...
      console.log(userInfo.id);
      const q =
        'UPDATE `heroku_713c4886f766b8c`.mentor_education SET `college` = ?, `college_yr_graduate` = ?, `high_school` = ?, `hs_yr_graduated` = ?  WHERE `mentor_id` = ?;';
...
const updateProfileMentorSocial = (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, userInfo) => {
    if (err) return res.status(500).json(err);
      ...
      console.log(userInfo.id);
      const q =
        'UPDATE `heroku_713c4886f766b8c`.mentors SET fb_link = ?, linked_in_link = ? WHERE id = ?;';
